...
class DataBaseManager():
    ...
    def update_materia(self, conn, materia):
        try:
            sql = '''UPDATE materias SET calificacion=?
            WHERE nombre=? and alumno=? and  id=? '''
            cur = conn.cursor()
            cur.execute(sql, materia)
            conn.commit()
            msg = "Registered Successfully"
    ...
    def is_an_existing_materia(self, conn, materia):
        ...
        :return:
        '''
        sql = '''SELECT * FROM materias WHERE id = ? and nombre=? and alumno=? and calificacion=?'''
        cur = conn.cursor()
        cur.execute(sql, materia)
